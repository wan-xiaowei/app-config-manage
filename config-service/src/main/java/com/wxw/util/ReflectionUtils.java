...
import com.wxw.annotation.DBLog;
...
public class ReflectionUtils {
	 ...
	 * 直接读取对象属性值, 无视private/protected修饰符, 不经过getter函数.
	 */
	public static String getLogAnnotationValue(final Object object, final String fieldName) {
		Field field = getDeclaredField(object, fieldName);
		...
		DBLog dbLog = null;
		try {
			dbLog = field.getAnnotation(DBLog.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(dbLog!=null){
	...
	/**
	 * 直接读取对象属性值和注解, 无视private/protected修饰符, 不经过getter函数.
	 */
	public static Object getFieldLabels(final Object object, final String fieldName, Annotation clazz) {
		Field field = getDeclaredField(object, fieldName);
		...
		try {
			result = field.get(object);
			Annotation annotation= field.getAnnotation(clazz.getClass());
		} catch (IllegalAccessException e) {
	...
	public static void main(String[] args) throws InvocationTargetException, IllegalAccessException {
		/*
		 * Document document = new Document(); document.setId(2); document.setCreateDate(new Date()); DocumentVo dcoVo = new DocumentVo();
		 * ReflectionUtils.copyPorperties(dcoVo, document,new String[]{"id","businessName","createDate","applyName","docTitle",
		 * "transactStatus"}); logger.info(dcoVo.getId());
		 */
	}
}